...
if __name__ == '__main__':
    ...
    agent.load(dirPath, 120) # 5 + 149 + 30

    step_total, episode_reward, done = 0, 0, False
    ...
    for ep in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            if step_total < 256:
                # action = np.array([np.clip(
                #     np.random.uniform(-1, 1), 0., ACTION_V_MAX),
                #     np.clip(np.random.uniform(-1, 1), -ACTION_W_MAX, ACTION_W_MAX)])
                # unnorm_action = np.array([action_unnormalized(action[0], ACTION_V_MAX, 0), action_unnormalized(action[1], ACTION_W_MAX, ACTION_W_MIN)])
            ...
            if step_total >= 256:
                ...
                for _ in range(num_updates):
                    agent.update(replay_buffer, step_total)


            next_obs, reward, done = env.step(unnorm_action)
            cv2.imwrite(dirPath + '/aqui1.png', obs[:3,:,:].reshape(100, 100, 3))
            cv2.imwrite(dirPath + '/aqui2.png', obs[3:6,:,:].reshape(100, 100, 3))
            cv2.imwrite(dirPath + '/aqui3.png', obs[6:9,:,:].reshape(100, 100, 3))
        ...
        print('reward average per ep: ' + str(episode_reward) + ' and break step: ' + str(step))
        
        if ep%5 == 0:
            agent.save(dirPath, ep)
            agent.save_curl(dirPath, ep)
